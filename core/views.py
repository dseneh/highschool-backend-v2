...
def validate_tenant_is_in_public_schema():
    ...
    if connection.schema_name != 'public':
        raise ValidationError({"detail": "Tenant operations must be performed in the public schema"})
    return True
...
def current_tenant(request):
    ...
    """
    if connection.schema_name == 'public':
        return Response({"detail": "No tenant context found (public schema)"}, status=400)
...
def search_tenant_info(request):
    """
    Search for tenant information by email, phone, or id_number.
    ...
    with schema_context(get_public_schema_name()):
        ...
        if email or id_number:
            ...
            users = User.objects.filter(user_filters).only(
                ...
                'username', 'account_type', 'is_active'
            )
            
            for user in users:
                results.append({
                    "user_type": "user",
                    "tenant": public_tenant_info,
                    "data": {
                        "id": str(user.id),
        ...
        for tenant in tenants:
            with schema_context(tenant.schema_name):
                ...
                students = Student.objects.filter(filters).select_related('grade_level').only(
                    'id', 'id_number', 'email', 'phone_number', 'first_name', 
                    'middle_name', 'last_name', 'gender', 'status', 'grade_level'
                )
                ...
                staff_members = Staff.objects.filter(filters).select_related('position').only(
                    'id', 'id_number', 'email', 'phone_number', 'first_name', 
                    'middle_name', 'last_name', 'gender', 'status', 'position', 'is_teacher'
                )
                ...
                for staff_member in staff_members:
                    results.append({
                        ...
                        "data": {
                            ...
                            "position": staff_member.position.title if staff_member.position else None,
