...
class AssessmentListCreateView(APIView):
    ...
    - include_stats: Optional. Set to 'true' to include grade statistics for each assessment
    """
    ...
    def get(self, request, gradebook_id):
        ...
        # return Response({
        #     "meta": meta, 
        #     "results": AssessmentOut(qs, many=True, context={'request': request}).data, 
        #     "gradebook": gradebook_details
        # })
        # page, meta = paginate_qs(qs, request)
        return Response(AssessmentOut(qs, many=True, context={'request': request}).data)
    ...
    def post(self, request, gradebook_id):
        ...
        if due_date:
            try:
                ...
                if not (marking_period.start_date <= due_date_obj <= marking_period.end_date):
                ...
                return Response({"detail": "Invalid date format. Use YYYY-MM-DD format."}, status=400)
        
        # Check for duplicate assessment names (using exists() for efficiency)
        if Assessment.objects.filter(gradebook_id=gradebook_id, name__iexact=name, marking_period_id=marking_period_id).exists():
            return Response({"detail": "An assessment with this name already exists in this gradebook."}, status=400)
        ...
        try:
            ...
            return Response({"detail": "Failed to create assessment. " + str(e)}, status=400)
